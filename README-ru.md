...
`--force` берёт нашу версию каждого файла. Это тоже не разрушительно: файл, содержимое которого больше не совпадает с манифестом, перед перезаписью копируется в `.claude/.upgrades/<путь>.mine`.
