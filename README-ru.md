...
`--force` берёт нашу версию каждого файла. Правила, агенты и блок в CLAUDE.md сначала копируются в `.claude/.upgrades/<путь>.mine`, так что заменяемая версия остаётся. Хуки и навык project-discovery заменяются без копии — это наш код, он и так перезаписывается при каждом запуске.
...
О: По каждому правленому правилу и агенту зададут вопрос, с показом различий по запросу; невыбранная версия ложится в `.claude/.upgrades/`. Хуки и навыки обновляются всегда. В `CLAUDE.md` обновляется только наш обрамлённый блок — остальное остаётся твоим. Флаги `--force` и `--keep-mine` отвечают за всё сразу.
